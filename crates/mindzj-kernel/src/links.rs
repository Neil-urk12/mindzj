use crate::types::{GraphData, GraphEdge, GraphNode, LinkType, NoteLink};
...
mod tests {
    ...
    fn test_heading_anchors() {
        let content = "Link to [[note#heading]] and [[folder/other#section|alias]].";
        let links = LinkIndex::parse_links("source.md", content);

        assert_eq!(links.len(), 2);
        // resolve_link_target strips the anchor
        assert_eq!(links[0].target, "note.md");
        assert_eq!(links[1].target, "folder/other.md");
        assert_eq!(links[1].display_text, Some("alias".to_string()));
    }

    #[test]
    fn test_multiple_links_on_one_line() {
        let content = "See [[a]] and also [[b]] here.";
        let links = LinkIndex::parse_links("source.md", content);
